...
Proof.
  intros x H H0. (*Se fizer só intros dá nomes automagically, mas é boa prática escolher os nomes*)
  apply H.
  assumption.
  assumption.
Qed.
...
Lemma ex1' : (P -> Q) -> ~Q -> ~P.
Proof.
  intros. (*O intros não dá unfold na negação a não ser que esteja nomeado*)
  unfold not. intro. (*o intro dá unfold na negaçao e aplica a regra, mas podemos definir o unfold explicitamente*)
  apply H0.
  apply H.
  assumption.
Qed.
...
Proof.
  ...
  destruct H as [H1 H2].
  exact H2.
  destruct H; assumption. (*destruct H as [H1 H2]; exact H1.*)
Qed.
...
Proof.
  intros.
  destruct H as [h1 | h2]. (*O \/ precisa da barra para fazer dois ramos, um para o P ser verdade, outro para o Q ser verdade*)
  - right. assumption.
  - left; assumption.
Qed.


(*Theorem é o mm que Lemma*)
